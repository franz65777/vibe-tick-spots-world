-- Update follow request notification trigger to:
-- 1. Support all 12 languages (not just en/it)
-- 2. Send a notification to the REQUESTER when their request is ACCEPTED, showing who accepted
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- Re-opened request (e.g. declined -> pending): create a NEW notification
    ...
    -- When request is ACCEPTED: notify the REQUESTER with acceptor's info
    IF NEW.status = 'accepted' AND OLD.status = 'pending' THEN
      -- Expire the original follow_request notification
      ...
      -- Get acceptor (requested_id) info
      SELECT p.username, p.avatar_url
        INTO acceptor_username, acceptor_avatar
      FROM public.profiles p
      WHERE p.id = NEW.requested_id;

      -- Get requester's language preference
      SELECT p.language
        INTO requester_lang
      FROM public.profiles p
      WHERE p.id = NEW.requester_id;

      -- Localized "accepted" notification (12 languages)
      CASE requester_lang
    ...
    -- When request is declined, expire its notifications
