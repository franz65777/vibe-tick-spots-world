...
const RewardsPage = () => {
  ...
  const handleClaim = (reward: Reward) => {
    if (coins < reward.cost) {
      toast.error(t('notEnoughCoins', { ns: 'profile', defaultValue: 'Not enough coins!' }));
      return;
    }
    // TODO: Implement actual claim logic
    toast.success(t('rewardClaimed', { ns: 'profile', defaultValue: 'Reward claimed! Check your email for details.' }));
  };
    ...
    <div className="min-h-screen bg-background overflow-y-auto">
      ...
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm border-b border-border">
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <h1 className="text-lg font-semibold">{t('rewards', { ns: 'profile', defaultValue: 'Rewards' })}</h1>
        </div>
      </div>
      ...
      <div className="px-4 py-6 border-b border-border">
        <div className="bg-gradient-to-r from-yellow-400 to-amber-500 rounded-2xl p-6 text-white shadow-lg">
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <p className="text-sm opacity-90">{t('yourBalance', { ns: 'profile', defaultValue: 'Your Balance' })}</p>
              <p className="text-3xl font-bold">{coins.toLocaleString()} {t('coins', { ns: 'profile', defaultValue: 'coins' })}</p>
            </div>
          </div>
          <p className="text-sm opacity-80 mt-2">
            {t('earnMoreHint', { ns: 'profile', defaultValue: 'Save places, complete challenges, and maintain your streak to earn more!' })}
          </p>
        </div>
      </div>
      ...
      <div className="px-4 py-6 pb-32">
        <h2 className="text-lg font-semibold mb-4">{t('availableRewards', { ns: 'profile', defaultValue: 'Available Rewards' })}</h2>
                      ...
                      >
                        {canAfford ? (
                          <>
                            ...
                            {t('claim', { ns: 'profile', defaultValue: 'Claim' })}
                          </>
                        ) : (
                          <>
                            <Lock className="w-4 h-4 mr-1" />
                            {t('locked', { ns: 'profile', defaultValue: 'Locked' })}
                          </>
                        )}
                      </Button>
                    </div>
        ...
        {/* Coming Soon */}
        <div className="mt-8 p-6 bg-muted/50 rounded-xl text-center border-t border-border pt-8">
          <Gift className="w-10 h-10 mx-auto text-muted-foreground mb-2" />
          <h3 className="font-semibold text-foreground">{t('moreComingSoon', { ns: 'profile', defaultValue: 'More Coming Soon!' })}</h3>
          <p className="text-sm text-muted-foreground mt-1">
            {t('moreComingSoonDesc', { ns: 'profile', defaultValue: 'We are working on more exciting rewards for you.' })}
          </p>
        </div>
      </div>
    </div>
