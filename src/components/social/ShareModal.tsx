...
export const ShareModal = ({ isOpen, onClose, onShare, postId }: ShareModalProps) => {
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast({
        title: t('linkCopiedTitle', { ns: 'common', defaultValue: 'Link copied' }),
        description: t('linkCopiedDesc', { ns: 'common', defaultValue: 'The link has been copied to your clipboard' }),
      });
      onClose();
    } catch {
      toast({
        title: t('error', { ns: 'common', defaultValue: 'Error' }),
        description: t('copyLinkError', { ns: 'common', defaultValue: 'Unable to copy the link' }),
        variant: 'destructive',
      });
    }
  };
  ...
  const handleWhatsAppShare = () => {
    ...
    const text = encodeURIComponent(
      t('whatsAppShareText', {
        ns: 'common',
        defaultValue: 'Check out this post on Spott: {{url}}',
        url: postUrl,
      })
    );
            ...
            <>
              <div className="flex items-center justify-between px-4 py-3 shrink-0">
                ...
                <h3 className="font-semibold text-base">
                  {t('shareTo', { ns: 'common', defaultValue: 'Share to' })}
                </h3>
                <div className="w-8" />
              </div>
                    ...
                    {filteredUsers.length === 0 ? (
                      <div className="text-center py-16 px-4">
                        <p className="text-muted-foreground text-sm mb-6">
                          {query
                            ? t('noUsersFound', { ns: 'common', defaultValue: 'No users found' })
                            : t('followToShare', { ns: 'common', defaultValue: 'Follow people to share with them' })}
                        </p>
                        <div className="flex justify-center gap-8 mt-8">
                          ...
                          <button onClick={handleCopyLink} className="flex flex-col items-center gap-2">
                            ...
                            <span className="text-xs font-medium">
                              {t('copyLink', { ns: 'common', defaultValue: 'Copy link' })}
                            </span>
                          </button>
                        </div>
                      </div>
              ...
              {selected.size > 0 && (
                <div className="p-4 shrink-0">
                  <Button onClick={handleSend} disabled={sending} className="w-full gap-2 rounded-full h-12 text-base">
                    {sending ? (
                      <>
                      ...
                      <>
                        {t('send', { ns: 'common', defaultValue: 'Send' })}{' '}
            ...
            <>
              <div className="px-4 py-6">
                <div className="grid grid-cols-4 gap-6">
                  <button onClick={handleWhatsAppShare} className="flex flex-col items-center gap-2 group">
                  ...
                  <button onClick={onClose} className="flex flex-col items-center gap-2 group">
                    ...
                    <span className="text-xs font-medium">
                      {t('addToStory', { ns: 'common', defaultValue: 'Add to story' })}
                    </span>
                  </button>

                  <button onClick={() => setShowUserList(true)} className="flex flex-col items-center gap-2 group">
                    <div className="w-14 h-14 rounded-full bg-muted flex items-center justify-center shadow-lg group-active:scale-95 transition-transform">
                      <Share2 className="w-6 h-6 text-foreground" />
                    </div>
                    <span className="text-xs font-medium text-center leading-tight">
                      {t('shareToEllipsis', { ns: 'common', defaultValue: 'Share to…' })}
                    </span>
                  </button>
                </div>
              </div>
