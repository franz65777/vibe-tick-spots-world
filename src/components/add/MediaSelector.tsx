...
export const MediaSelector: React.FC<MediaSelectorProps> = ({
  ...
  return <div className="space-y-3" data-photo-selection="false">
      <div className="grid grid-cols-2 gap-3">
        ...
        {selectedFiles.length < maxFiles && <button onClick={handleClick} className="aspect-square border-2 border-dashed border-border rounded-xl flex flex-col items-center justify-center hover:border-primary hover:bg-primary/5 transition-colors">
            <ImageIcon className="w-8 h-8 text-muted-foreground mb-2" />
            <span className="text-sm text-muted-foreground">{t('addMore', {
            ns: 'add'
          })}</span>
          </button>}
      </div>
