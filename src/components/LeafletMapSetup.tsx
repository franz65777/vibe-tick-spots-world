...
const LeafletMapSetup = ({
  ...
  // IMPORTANT: Do NOT include mapCenter in dependencies - it changes on every render and causes infinite loops.
  // We use a ref to capture the latest mapCenter value when recenterToken changes.
  ...
  useEffect(() => {
    ...
    console.log('🗺️ Centering map to:', center, 'token:', recenterToken);
      ...
      {selectedPlace && !selectedPostFromPin && (
        <PinDetailCard
          place={{
            ...
            fromList: (selectedPlace as any).fromList,
          }}
          ...
          onBack={
            fromMessages && onBackToMessages
              ? onBackToMessages
              : (selectedPlace as any)?.returnTo || (selectedPlace as any)?.fromList
                ? (() => onCloseSelectedPlace?.())
                : undefined
          }
        />
