...
const MapSection = ({ 
  ...
  return (
    <>
      <div className={`${isExpanded ? 'fixed inset-0 w-screen h-screen relative' : 'flex-1 relative h-full -mb-[calc(4rem+env(safe-area-inset-bottom))]'} w-full overflow-hidden`}>
        ...
        {!isListViewOpen && (
        <div className={`${isExpanded ? 'fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)]' : 'absolute bottom-[calc(4rem+env(safe-area-inset-bottom)-1.75rem+5px)]'} right-3 z-[1000] flex flex-row gap-2 data-[has-sharing=true]:flex-col data-[has-sharing=true]:items-end`} data-has-sharing={false}>
